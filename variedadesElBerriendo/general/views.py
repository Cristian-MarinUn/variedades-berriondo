...
def checkout(request):
    ...
    preferred_address = Address.objects.filter(user=request.user, is_preferred=True).first()
    payment_method = PaymentMethod.objects.filter(user=request.user, is_preferred=True).first()

# Si no hay dirección preferida, mostrar un mensaje
    address_display = preferred_address.address if preferred_address else "No hay dirección registrada"

# Si no hay método de pago, mostrar un mensaje
    if payment_method:
        censored_number = f'**** **** **** {payment_method.card_number[-4:]}' if len(payment_method.card_number) >= 4 else '**** **** **** ****'
    else:
        censored_number = "No hay método de pago registrado"

    context = {
        'total_price': total_price,
        'form': form,
        'address': address_display,
        'payment': censored_number
    }
...
def procesar_compra(request, user_id):
    carrito = Carrito.objects.filter(usuario_id=user_id)
    ...
    for item in carrito:
        ...
        # Registrar los ítems comprados
        items.append(f"{item.producto.nombre} (x{item.cantidad})")
